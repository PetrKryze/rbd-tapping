...
if strcmp(Whiskers,'box')
    ...
    %yMean=mean(yValues);
    yMean=median(yValues);
    %yStd=std(yValues);
    yStd=iqr(yValues);
    yQ = quantile(yValues,[0.25 0.75]);
    
    ySem=yStd/sqrt(size(yValues,1));
    yCI=ySem*1.96;
    %plot the standard deviation box
    rectangle('Position',[i-Width/WhiskersWidthRatio,yQ(1),2*Width/WhiskersWidthRatio,yQ(2)-yQ(1) ],'FaceColor',StdColor(StdIndex,:),'EdgeColor', [0 0 0] ,'LineWidth',1.5);
